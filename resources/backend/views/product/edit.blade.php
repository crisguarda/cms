                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="card-title">Aqui pode editar o seu Produto</h4>
                            <br>
                            <form method="post" action="{{ route('product.update', $product->id) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-select @error('product_category_id') is-invalid @enderror" aria-label="Default select example" name="product_category_id">
                                            ...
                                            @foreach($productCategories as $category)
                                                <option value="{{ $category->id }}" @if($category->id == old('product_category_id') || $category->id == $product->product_category_id) selected @endif>{{ $category->title }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control @error('title') is-invalid @enderror" type="text" placeholder="Titulo" id="title" name="title" value="{{ old('title') ?? $product->title }}">
                                        @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea class="form-control @error('desc') is-invalid @enderror" id="elm1" name="desc">{{ old('desc') ?? $product->desc}}</textarea>
                                        @error('desc')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                                ...
                                                <div class="">
                                                    <select class="form-select" aria-label="Default select example" name="unity[]" value="${last_line}">
                                                        ...
                                                        @foreach($unities as $unity)
                                                            <option @if($unity->id === $unity_price->unity_id) selected @endif value="{{ $unity->id }}">{{ $unity->unity }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <label class="col-form-label">Unidade</label>
                                                <div class="">
                                                    <input value="{{ $unity_price->price }}" class="form-control" type="number" step='.01' placeholder="Preço" id="price" name="price[]">
                                                </div>
                                                <a class="btn btn-danger sm remove_line" id="delete_line_{{ $unity_price->id }}" title="Remover Linha"><i class="fas fa-trash-alt"></i></a>
                                            </div>
                                        @endforeach
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <img
                                            ...
                                            src="{{ url($product->image != 0 ? $product->image : 'upload/no__image.png') }}"
                                            alt=""
                                        >
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control @error('image_alt') is-invalid @enderror" type="text" placeholder="Alt Imagem" id="image_alt" name="image_alt" value="{{ old('image_alt') ?? $product->image_alt }}">
                                        @error('image_alt')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3 form-switch">
                                    <label class="form-check-label col-sm-2 col-form-label" for="is_new">Produto Novo</label>
                                    <div class="col-sm-10">
                                        @php
                                            $value = old('is_new') ?? $product->is_new;
                                        @endphp
                                        <input type="hidden" name="is_new" value="0">
                                        <input type="checkbox" class="form-check-input mx-0 @error('is_new') is-invalid @enderror" id="is_new" name="is_new" @if($value) checked @endif value="1">
                                        @error('is_new')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3 form-switch">
                                    ...
                                    <div class="col-sm-10">
                                        @php
                                            $value = old('is_promo') ?? $product->is_promo;
                                        @endphp
                                        <input type="hidden" name="is_promo" value="0">
                                        <input type="checkbox" class="form-check-input mx-0 @error('is_promo') is-invalid @enderror" id="is_promo" name="is_promo" @if($value) checked @endif value="1">
                                        @error('is_promo')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control @error('order') is-invalid @enderror" type="number" placeholder="Ordem" id="order" name="order" value="{{ old('order') ?? $product->order }}">
                                        @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3 form-switch">
                                    ...
                                    <div class="col-sm-10">
                                        @php
                                            $value = old('active') ?? $product->active;
                                        @endphp
    ...
    <script>
        $(document).ready(function (){
            ...
            $('.remove_line').click(function (e){
                $(this).closest('.unity_price_line').remove();
            });
        });
    </script>
@endsection
